...
# Create ONE global env and register the filter
_TEMPLATE_DIR = str(AGENT_PROMPTS.parent)
_TEMPLATE_NAME = AGENT_PROMPTS.name
JINJA_ENV = Environment(loader=FileSystemLoader(_TEMPLATE_DIR), autoescape=False)
JINJA_ENV.filters["fromyaml"] = fromyaml
# per-dir Jinja env cache so we don't rebuild on every call
...
def _get_env_for_dir(dir_path: str) -> Environment:
    env = _ENV_CACHE.get(dir_path)
    if env is None:
        env = Environment(loader=FileSystemLoader(dir_path), autoescape=False)
        env.filters["fromyaml"] = _fromyaml  # register filter on THIS env
        _ENV_CACHE[dir_path] = env
    return env
...
def _load_section(
    ...
    **kwargs: Any,
    ...
    Args:
        section: Section name in the YAML (e.g., "planner", "summarizer").
        template_path: Path to the .yaml.j2 template to render.
        **kwargs: Variables to inject into the template.

    Returns:
        Dict[str, Any]: The mapping under the requested section.
    """
    p = Path(template_path)
    template_dir = str(p.parent)
    template_name = p.name

    jinja_env = _get_env_for_dir(template_dir)
    template = jinja_env.get_template(template_name)
    rendered = template.render(**kwargs)

    cfg = yaml.safe_load(rendered) or {}
    if section not in cfg:
        raise KeyError(f"Section '{section}' not found in template file: {p}")
    return cfg[section]
