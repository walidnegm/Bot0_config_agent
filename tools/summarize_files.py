...
def extract_kv_lines(path: Path) -> Dict[str, Any]:
    try:
...
def summarize_files(files: List[str]) -> Dict[str, Any]:
    if not files:
        return {"status": StepStatus.ERROR, "message": "No files provided. Please specify files as a list."}
